    ...
    <div className="max-w-6xl mx-auto px-4 py-20">
      ...
      <section className="mb-16">
        ...
        <div className="space-y-6">
          {education.map(edu => (
            <div key={edu.id} className="bg-slate-800/50 p-6 rounded-lg border border-slate-700">
              <h3 className="text-2xl font-bold text-blue-400 mb-2">{edu.degree}</h3>
              <p className="text-xl text-slate-300 mb-2">{edu.institution}</p>
      ...
      <section className="mb-16">
        ...
        <div className="space-y-6">
          {certifications.map(cert => (
            <div key={cert.id} className="bg-slate-800/50 p-6 rounded-lg border border-slate-700">
              <h3 className="text-2xl font-bold text-blue-400 mb-2">{cert.name}</h3>
              <p className="text-xl text-slate-300 mb-2">{cert.issuer}</p>
              <p className="text-slate-400 mb-3">{cert.date}</p>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {courses.map(course => (
            <div key={course.id} className="bg-slate-800/50 p-4 rounded-lg border border-slate-700">
              <p className="text-slate-300 font-semibold">{course.name}</p>
            </div>
          ))}
        </div>
      </section>
